...
namespace Dominio
{
    public class Sistema
    {
        ...
        #region PrecargasRequeridas
        private void PrecargasRequeridas() {
            try
            {
                FinalizarPartidoPorId(1);
                FinalizarPartidoPorId(2);
                FinalizarPartidoPorId(3);
        ...
        private void PrecargarPeriodistas()
        {
            try
            {
                AltaPeriodista(new Periodista("Victor", "Hugo", "victor@gmail.com", "12345678"));
                AltaPeriodista(new Periodista("Julio", "Hernandez", "julio@gmail.com", "12356748"));
                AltaPeriodista(new Periodista("Yuriria", "Sierra", "yi@gmail.com", "45623781"));
                AltaPeriodista(new Periodista("Bruno", "Hugo", "bruno@gmail.com", "45623755"));
            }
            catch (Exception ex)
            {
        ...
        #region Precarga Operadores
        private void PrecargarOperadores()
        {
            try
            {
                AltaOperador(new Operador("juanma", "ximenez", "juanma@gmail.com", "12345678", DateTime.Parse("2022-11-18")));
                AltaOperador(new Operador("Pedro", "xeneimz ", "pedro@gmail.com", "12356748", DateTime.Parse("2022-11-18")));
                AltaOperador(new Operador("Yuri", "riva ", "yuri@gmail.com", "45623781", DateTime.Parse("2022-11-18")));
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        #region PrecargarReseñas


        private void PrecargarReseñas()
        {
            try
            {   
                AltaReseña(new Resenia(DateTime.Parse("2022-11-18"), BuscarPartidoPorID(1), "Goleada Argentina a ecuador triplete de messi", "la agarro messi y fh para los ecuatorianos"), BuscarUsuarioPorEmail("victor@gmail.com") as Periodista);
        ...
        public void AltaPeriodista(Usuario p)
        {
            try
            {
                if (p == null) throw new Exception("El periodista no puede ser nulo");
                p.Validar();
                if (BuscarUsuarioPorEmail(p.Email) != null) throw new Exception("Ya existe un usuario con ese email");
                _usuarios.Add(p);
            }
            catch (Exception e)
            {
        ...
        public void AltaOperador(Usuario o)
        {
            try
            {
                if (o == null) throw new Exception("El Operador no puede ser nulo");
                o.Validar();
                if (BuscarUsuarioPorEmail(o.Email) != null) throw new Exception("Ya existe un usuario con ese email");
                _usuarios.Add(o);
            }
            catch (Exception e)
            {
        ...
        public void AltaReseña(Resenia resenia, Periodista periodista)
        {
            try
            {
                if (periodista == null) throw new Exception("El periodista no puede ser nulo");
                periodista.IngresarReseña(resenia);
            }
            catch (Exception e)
            {
        ...
        public Usuario BuscarUsuarioPorEmail(string email)
        {
            ...
            {
                if (_usuarios[i].Email.Equals(email)) buscado = _usuarios[i];
                i++;
            }
        ...
        public void FinalizarPartidoPorId(int idPartido)
        {
            try
            {
                Partido p = BuscarPartidoPorID(idPartido);
                p.FinalizarPartido();
            }
            catch (Exception e)
            {
        ...
        {
            List<Partido> listado = new List<Partido>();
            foreach (Partido p in _partidos)
            {
        ...
        public List<Partido> ListarPartidosFinalizadosEntreDosFechas(DateTime f1, DateTime f2)
        {
            List<Partido> listado = new List<Partido>();
            foreach (Partido p in _partidos)
            {
                if (p.Finalizado == true && p.ElPartidoEstaEntreDosFechas(f1, f2))
                {
                    listado.Add(p);
                }
            }
        ...
        public List<Partido> ListarPartidosActivos()
        {
            List<Partido> listado = new List<Partido>();
            foreach (Partido p in _partidos)
            {
        ...
        public Usuario LoginUsuario(string email, string password)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(email) || string.IsNullOrEmpty(password)) throw new Exception("Los datos son incorrectos");
                int i = 0;
                while (buscado == null && i < _usuarios.Count)
                {
                    if (_usuarios[i].Email.Equals(email.ToLower()) && _usuarios[i].Password.Equals(password)) buscado = _usuarios[i];
                    i++;
                }
            }
            catch (Exception ex)
        ...
        public List<Resenia> ListarReseñasPorPeriodista(Periodista p)
        {
            List<Resenia> listado = new List<Resenia>();
            try
            {
                if (p == null) throw new Exception("No se ha encontrado el periodista");
                foreach (Resenia r in p.Resenias)
                {
                    listado.Add(r);
                }
            }
        ...
        public List<Periodista> VisualizarPeriodistas()
        {
            List<Periodista> aux = new List<Periodista>();
            foreach (Usuario u in _usuarios)
            {
                if (u is Periodista)
                {
                    Periodista p = u as Periodista;
                    aux.Add(p);
                }
            }
        ...
        public List<Partido> BuscarPartidosConTarjetaRojaQueReseñoUnPeriodista(string email)
        {
            Periodista p = BuscarUsuarioPorEmail(email) as Periodista;
            if (p == null) throw new Exception("No existe un periodista con ese email");
            List<Partido> aux = new List<Partido>();
            foreach (Resenia r in p.Resenias)
            {
                if (r.Partido.SacaronRoja()) aux.Add(r.Partido);
            }
            return aux;
        }

        public int GolesPorSeleccion(Seleccion s)
        {
            int goles = 0;
            foreach (Partido p in _partidos)
            {
                ...
                {
                    if (i.Jugador.Pais.Equals(s.Pais) && i.Inc.Equals("Gol"))
                    {
                        goles++;
                    }
                }
        ...
        public List<Seleccion> SeleccionesConMasGoles()
        {
            List<Seleccion> seleccionesGoleadoras = new List<Seleccion>();
            int cantGoles = 0;
            foreach (Seleccion s in _selecciones)
            {
                if (GolesPorSeleccion(s) > cantGoles)
                {
                    cantGoles = GolesPorSeleccion(s);
                    seleccionesGoleadoras.Clear();
                    seleccionesGoleadoras.Add(s);
                }

                if (GolesPorSeleccion(s) == cantGoles && !seleccionesGoleadoras.Contains(s))
                {
                    seleccionesGoleadoras.Add(s);
                }
            }

            return seleccionesGoleadoras;
        }
